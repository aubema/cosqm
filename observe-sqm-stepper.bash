...
#
findSQM () {
     avgnum=10 # number of scan to average
     nscan=1
     declare -a minim
     nmoy=1
     let pauseflag=0 
	...
	echo "Searching number of steps: " $nstep "Skipping " $movestep "tics" 
	   ...
	   let seuil100=seuil*100
	   let maxim100=maxim*100
	   if [ $meas -lt $seuil100 ] && [ $meas -gt $maxim100 ] 
           then pauseflag=1
	   fi
	   echo $n $pos $meas
           if [ $meas -lt $memoi ]
	...
	let nscan=nscan+1
	let finalsqm=finalsqm+possqm
     done
     let possqm=finalsqm/avgno
     echo "Average SQM position:" $possqm
}
# ==================================
...
# should write 2030
minim=1200 # minimal value of the interval of sky brightness optimal to find SQM position
maxim=1250 # maximal value of the inverval of sky brightness optimal to find SQM position 
#
# set band list
...
do    findIntegration
      ...
      do findIntBrightness
	 echo "Wait 15 min until a bit of darkness"
	 sleep 900
      done
      if [ $meas -lt $maxim ]
      then findSQM
